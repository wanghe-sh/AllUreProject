define([
    ...
    productListApp.controller('ListController', function ($scope, $attrs, $http, $location) {
        ...
        $scope.priceSection = function (detail) {
            //add search key in list
            if ($scope.searchInListKey != undefined && $scope.searchInListKey.length !=0) {
                if (detail.Price > $scope.priceGap.minPrice && (detail.Price <= $scope.priceGap.maxPrice || !$scope.priceGap.maxPrice) && (detail.Name.indexOf($scope.searchInListKey) >0)) {
                    return true;
                }
            }
            else {
    ...
    productListApp.controller('SubscribedMailController', SubscribedMailController);
